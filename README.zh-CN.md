...
| 游戏 | 状态 | 人数 / 时长 |
|---|---|---|
| **Who's Undercover?（谁是卧底）** | ✅ 开发中（首发） | 3–24 人 / 5–10 分钟 |
...
- **Auto-Simulate（自动模拟）** — 一个人、上帝视角：用真实规则引擎 + 机器人自动跑完整一局（可播放/暂停/单步/调速），方便自己预演或给别人讲流程。

### Live Room — 你们打算怎么玩

设置页先问「你们打算怎么玩？」，再映射到底层房间模型——人少不用纠结"组数×人数"：

- **每人独立（Individuals）** — 所有人各自参赛、共玩一局（3–24 人），找出唯一的卧底。每个人用自己的名字显示，不再叫"Group N"。
- **组间对抗（Teams vs teams）** — 分成若干队，每队共享同一个词，以队为单位发言/投票（先到先锁一票），找出卧底队。
- **多组并行（Parallel groups）** — 每个小组各自玩一局、同时进行，各自找出组内卧底。

在**组间对抗**和**多组并行**里，玩家加入时**自己选组**（也可点"自动分配"帮你平衡），并且**开局前可以随时换组**。

### Host 现场控制（无需重开房间）

- 调整**卧底人数**、开关 **Mr White**、换**词库**和**计时器**——下一轮生效。
- 现场**扩/缩容**（组数、每组人数上限）——但绝不会缩到比"已经有人坐进去的组"还少。
- **踢除玩家**；随时让新同学扫码加入。
...
- 四个小组用四种固定颜色贯穿全程（便于投影上一眼分辨）：
  - `Group A` 红 · `Group B` 蓝 · `Group C` 黄 · `Group D` 绿——再多的组（E … X，最多 24 组）用紫色系兜底。
- 排版清晰、留白充足，信息层级明确。
